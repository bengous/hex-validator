...
export async function validateContractTests(cwd = process.cwd()): Promise<ValidationResult> {
  ...
  for (const portFile of ports) {
    ...
    if (!fs.existsSync(testFile)) {
      missing.push({
        port: path.relative(cwd, portFile),
        expectedTest: path.relative(cwd, testFile),
      });
    }
  }
