...
describe('Validator E2E', () => {
  ...
  it('should detect every dependency-cruiser preset rule in the violation gallery', async () => {
    let stdout = '';
    ...
    // Each preset rule is exercised by a dedicated file in the invalid fixture.
    const presetRules = [
      'no-circular',
      'not-to-test',
      'mocks-live-in-infrastructure-mocks',
      'core-must-be-pure',
      ...
      'ui-layer-boundaries',
      'app-shell-layer-boundaries',
      'app-shell-domain-types-only',
      'no-client-components-to-composition',
      'no-cross-module-internals',
      'core-cross-module-domain-only',
    ];

    const depCruiserResult = parsed.results.find(
      (result: { name: string }) => result.name === 'Architecture (dependency-cruiser)'
    );
    expect(depCruiserResult).toBeDefined();
    const firedRules = depCruiserResult.artifacts.dependencyCruiser.groups.map(
      (group: { rule: string }) => group.rule
    );
    for (const rule of presetRules) {
      expect(firedRules).toContain(rule);
    }

    // Structural diagnostics from the plugin side (billing module is incomplete).
