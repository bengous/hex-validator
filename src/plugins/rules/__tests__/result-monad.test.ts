...
describe('resultMonadPlugin', () => {
  ...
  describe('Rule 1: .ok Discriminant Access', () => {
    it('allows .ok === comparison as a discriminant', async () => {
...
if (result.ok === true) {
  ...
  return result.value;
    ...
    it('allows .ok in logical operators', async () => {
...
console.log(error);`;
    ...
    it('accepts .error access with nearby guard', async () => {
...
return user;`;
  ...
  describe('Scope filtering', () => {
    ...
    it('respects scope=staged', async () => {
      ...
      mockReadFileSyncFn.mockReturnValue('const user = result.value;');
...
if (result.ok === true) {
  ...
  const user = result.value;
    ...
    it('does not flag non-Result Response.ok in regex fallback', async () => {
      const content = `const response = await fetch('/api');
if (response.ok) {
  return response;
}`;
