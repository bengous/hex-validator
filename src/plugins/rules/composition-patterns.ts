...
function checkFactoryNaming(relPath: string, content: string, findings: Finding[]) {
  ...
  for (const match of matches) {
    ...
    // Allow 'register*Handlers' pattern for handler registration.
    ...
    push(findings, {
      ...
      message: `Factory function '${functionName}' should start with 'create' prefix or match 'register*Handlers' pattern.`,
    ...
    for (const newExpr of newExpressions) {
      ...
      if (typeName.endsWith('Adapter') || typeName.endsWith('Repository')) {
        ...
        push(findings, {
          ...
          message: `Direct instantiation of '${typeName}' in route bypasses composition layer.`,
...
/**
 * Barrel Policy: forbid barrels inside hexagonal modules.
 *
 * Package-level public API files such as src/index.ts are outside this rule.
 */
