...
function getSuggestion(propertyName: string): string {
  switch (propertyName) {
    case 'ok':
      return 'Use .ok only as a narrowing discriminant.';
    case 'value':
    ...
    try {
      ...
      if (!isSafe) {
        ...
        push(findings, {
          ...
          message: `Unsafe .${propertyName} access on Result<T, E>.`,
...
function checkErrorPropertyUsage(relPath: string, content: string, findings: Finding[]) {
  ...
  for (const match of matches) {
    ...
    const hasGuardAbove =
      before.slice(Math.max(0, before.length - 200)).includes('Result.isErr(') ||
      before.slice(Math.max(0, before.length - 200)).includes('isErr(');
    ...
    push(findings, {
      ...
      message: 'Access .error only after Result.isErr() type guard.',
  ...
  for (const sf of project.getSourceFiles()) {
    ...
    for (const throwStmt of throwStatements) {
      ...
      push(findings, {
        ...
        message: `${matchedPath.layer} must not throw exceptions - return Result.fail() instead.`,
    ...
    if (ctx.scope === 'full' || files.length > 0) {
      ...
      if (useASTCheck) {
        ...
        for (const relPath of files) {
          ...
          // Keep limited regex fallback for test environments. Without type info,
          // do not flag .ok so Response.ok and other non-Result values stay safe.
