  ...
  for (const entry of FORBIDDEN_TOP_LEVEL) {
    ...
    if (fs.existsSync(absolutePath)) {
      ...
      push(findings, {
        ...
        file: relPath,
        code: entry.code,
        message: `Module ${moduleName} must not contain ${entry.folder}/.`,
  ...
  for (const entry of FORBIDDEN_NESTED) {
    ...
    if (fs.existsSync(absolutePath)) {
      ...
      push(findings, {
        ...
        file: relPath,
        code: entry.code,
        message: `Module ${moduleName} must not contain ${display}/.`,
        suggestion: entry.messageSuffix,
      });
    }
  }
  ...
  for (const folder of requiredFolders) {
    ...
    if (!fs.existsSync(folderPath)) {
      push(findings, {
        ...
        suggestion: 'Modules must follow the canonical hexagonal structure.',
      });
    }
  }
}
  ...
  for (const file of adapterFiles) {
    ...
    if (!/from\s+['"].*\/(?:application|core)\/ports/.test(content)) {
      push(findings, {
        ...
        suggestion: 'Adapters must implement port interfaces per dependency inversion.',
      });
    }
